...
class Partners_model extends CI_Model
{
    ...
    public function getPartnersYCantidadDeDesafiosCompartidosDeUsuariosActivos()
    {
        $query = $this->db->select('
            ...
            COUNT(rec.id) as desafios_compartidos
        ')
            ->from('usuarios as u')
            ...
            ->join('recomendaciones as rec', 'rec.partner_id = u.id', 'left')
            ->join('usuarios as em', 'em.id = rec.empresa_id', 'left')
            ->join('usuarios as st', 'st.id = rec.startup_id', 'left')

            ->where('u.rol_id', ROL_PARTNER)
            
            ->group_start()//estado empresa sea activo
            ->where('em.estado_id', USR_ENABLED)
            ->or_where('em.estado_id', USR_VERIFIED)
            ->group_end()
            
            ->group_start()//estado startup sea activo
            ->where('st.estado_id', USR_ENABLED)
            ->or_where('st.estado_id', USR_VERIFIED)
            ->group_end()
            ->group_by('u.id')
            ->get()
            ->result();
